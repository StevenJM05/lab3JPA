...
public class ActualizarLibro extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ActualizarLibro</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ActualizarLibro at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Libros libro = librosController.findLibros(id);
            if (libro != null) {
                libro.setTitulo(titulo);
                libro.setAutor(autor);
                libro.setGenero(genero);
                try {
                    librosController.edit(libro);
            ...
            request.setAttribute("errorMessage", "Ocurrió un error al actualizar el libro.");
            request.getRequestDispatcher("errorPage.jsp").forward(request, response);
        }
